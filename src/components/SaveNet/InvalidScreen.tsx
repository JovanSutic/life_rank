...
function InvalidScreen({ cityName }: { cityName: string }) {
  const { data, isError, isLoading, isFetching, error } = useCalculatorCountries();
  return (
    <AsyncStateWrapper
    ...
    >
      <div className="flex flex-col items-center justify-center pt-16">
        ...
        <p className="mb-4">
          At the moment, we have net & save data for cities in these countries:
        </p>
        ...
        <Link
          ...
          className="w-full md:w-[320px] cursor-pointer bg-blue-500 hover:bg-blue-600 text-white text-center py-2 px-6 rounded-lg transition-colors mt-4"
        >
          Back on the Map
        </Link>
      </div>
